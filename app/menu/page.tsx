...
function MenuPage() {
  ...
  const confirmBillout = () => {
    ...
    setTimeout(() => {
      setTables((prev) => {
        const n = { ...prev };
        if (activeTable) delete n[activeTable];
        return n;
      });
      setCart({});
      setBillPaid(false);
  ...
  return (
    <>
          ...
          {ssSlides.map((slide, i) => (
            <div
              ...
              style={{
                ...
                opacity: i === currentSlide ? 1 : 0, transition: "opacity 1.2s ease",
        ...
        {activeTable ? (
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <button
              ...
              disabled={!canBillOut}
              style={{
                background: canBillOut ? "linear-gradient(135deg,#1C0A00,#3D1200)" : "#E5E7EB",
                color: canBillOut ? "#FED7AA" : "#9CA3AF",
                border: "none", padding: "7px 16px", borderRadius: 20,
                fontFamily: "Syne", fontWeight: 700, fontSize: 13,
                cursor: canBillOut ? "pointer" : "default",
            ...
            <div style={{ background: "linear-gradient(135deg,#FFF7ED,#FED7AA)", border: "1.5px solid #F97316", borderRadius: 14, padding: "0.9rem 1.25rem", marginBottom: "1.25rem", display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: "#9A6046", marginTop: 2 }}>
                  Add <code style={{ color: "#EA580C" }}>?tableNo=3</code> to the URL to assign a table, e.g. <code style={{ color: "#EA580C" }}>localhost:3000/menu?tableNo=3</code>
                </div>
              </div>
            </div>
          )}
            ...
            {filteredItems.map((item) => {
              ...
              return (
                <div
                  ...
                  style={{ background: "#fff", borderRadius: 14, border: "1px solid #F3D5BE", overflow: "hidden", cursor: "pointer" }}
                >
                  <div style={{ height: 136, background: item.bg, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
                    ...
                    {item.badge && (
                      <div style={{ position: "absolute", top: 8, left: 8, background: "#F97316", color: "#fff", fontSize: 10, padding: "2px 9px", borderRadius: 20, fontWeight: 700 }}>
                        {item.badge}
                      </div>
                    )}
                  </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      ...
                      {qty > 0 ? (
                        <div style={{ display: "flex", alignItems: "center", gap: 5, background: "#FFF7ED", borderRadius: 20, padding: "2px 7px" }}>
                          <button
                            onClick={(e) => { e.stopPropagation(); changeCartQty(item.id, -1); }}
                            style={{ width: 20, height: 20, borderRadius: "50%", border: "1.5px solid #F97316", background: "#fff", cursor: "pointer", fontSize: 12, color: "#F97316", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}
                          >−</button>
                          <span style={{ fontSize: 12, fontWeight: 700, minWidth: 14, textAlign: "center", color: "#1C0A00" }}>{qty}</span>
                          <button
                            onClick={(e) => { e.stopPropagation(); changeCartQty(item.id, 1); }}
                            style={{ width: 20, height: 20, borderRadius: "50%", border: "none", background: "#F97316", cursor: "pointer", fontSize: 12, color: "#fff", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}
                          >+</button>
                        </div>
                      ) : (
                        <button
                          onClick={(e) => { e.stopPropagation(); addToCart(item); }}
                          style={{ background: "#F97316", color: "#fff", border: "none", width: 28, height: 28, borderRadius: "50%", fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
                        >+</button>
                      )}
                    </div>
                  </div>
        ...
        <div style={{ paddingTop: "0.5rem" }}>
          <div style={{ background: "#fff", borderRadius: 18, border: "1.5px solid #F3D5BE", overflow: "hidden", position: "sticky", top: 72, boxShadow: "0 8px 32px rgba(249,115,22,0.09)" }}>
            ...
            <div style={{ background: "linear-gradient(135deg,#F97316,#EA580C)", padding: "1rem 1.5rem", color: "#fff", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                <div style={{ fontFamily: "Syne", fontWeight: 800, fontSize: "1.05rem" }}>
                  {activeTable || "No Table Selected"}
                </div>
                <div style={{ fontSize: 12, opacity: 0.85, marginTop: 1 }}>
                  {activeTable
                    ? tableItemCount > 0
                      ? `${tableItemCount} item${tableItemCount !== 1 ? "s" : ""} across ${tableData?.orders.length ?? 0} batch${(tableData?.orders.length ?? 0) !== 1 ? "es" : ""}`
                      : "No orders yet — add items below"
                    : "Visit URL with ?tableNo=X"}
                </div>
              </div>
              {canBillOut && (
              ...
              <div style={{ paddingTop: tableData && tableData.orders.length > 0 ? 0 : "1rem", paddingBottom: "0.5rem" }}>
                <div style={{ fontSize: 11, letterSpacing: 2, textTransform: "uppercase", color: "#9A6046", fontWeight: 600, marginBottom: "0.5rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  {cartEntries.length > 0 && (
                    <button
                      onClick={() => setCart({})}
                      style={{ fontSize: 11, color: "#9A6046", background: "#FFF7ED", border: "none", cursor: "pointer", fontFamily: "DM Sans", padding: "2px 9px", borderRadius: 20 }}
                    >
                      Clear
                    </button>
                  )}
                  ...
                  <>
                    {cartEntries.map(({ item, qty }) => (
                      <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid #FDE8D8" }}>
                        <div style={{ fontSize: "1.4rem", width: 42, height: 42, background: "linear-gradient(135deg,#FFF7ED,#FED7AA)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                          {item.icon}
                        </div>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 3 }}>
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 4, background: "#FFF7ED", borderRadius: 20, padding: "2px 6px" }}>
                            <button
                              ...
                              style={{ width: 20, height: 20, borderRadius: "50%", border: "1.5px solid #F97316", background: "#fff", cursor: "pointer", fontSize: 11, color: "#F97316", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}
                            >−</button>
                            <span style={{ fontSize: 12, fontWeight: 700, minWidth: 14, textAlign: "center", color: "#1C0A00" }}>{qty}</span>
                            <button
                              onClick={() => changeCartQty(item.id, 1)}
                              style={{ width: 20, height: 20, borderRadius: "50%", border: "none", background: "#F97316", cursor: "pointer", fontSize: 11, color: "#fff", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}
                            >+</button>
                          </div>
                        </div>
                      </div>
            ...
            <div style={{ padding: "0.9rem 1.5rem 1.4rem", borderTop: "1px solid #FDE8D8", display: "flex", flexDirection: "column", gap: 8 }}>
              <button
                ...
                style={{
                  width: "100%",
                  background: canPlaceOrder ? "linear-gradient(135deg,#F97316,#EA580C)" : "#E5E7EB",
                  color: canPlaceOrder ? "#fff" : "#9CA3AF",
                  border: "none", padding: "12px", borderRadius: 12,
                  fontFamily: "Syne", fontWeight: 700, fontSize: 14,
                  cursor: canPlaceOrder ? "pointer" : "default", letterSpacing: "0.4px",
                }}
              >
              ...
              <button
                onClick={() => { if (canBillOut) setShowBillout(true); }}
                disabled={!canBillOut}
                style={{
                  width: "100%",
                  background: canBillOut ? "#fff" : "#F9FAFB",
                  color: canBillOut ? "#EA580C" : "#9CA3AF",
                  border: canBillOut ? "2px solid #F97316" : "2px solid #E5E7EB",
              ...
              >
                ...
                {canBillOut && (
                  <span style={{ background: "#FFF7ED", color: "#EA580C", borderRadius: 8, fontSize: 12, padding: "1px 8px", fontWeight: 700 }}>
                    ${tableGrand.toFixed(2)}
                  </span>
                )}
              </button>
            </div>
      ...
      {showBillout && activeTable && tableData && (
        <div
          onClick={() => { if (!billPaid) setShowBillout(false); }}
          style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(28,10,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem", backdropFilter: "blur(4px)" }}
        >
          <div
            onClick={(e) => e.stopPropagation()}
            style={{ background: "#fff", borderRadius: 24, width: "100%", maxWidth: 500, overflow: "hidden", boxShadow: "0 24px 64px rgba(249,115,22,0.25)" }}
          >
            {billPaid ? (
              ...
              <>
                <div style={{ background: "linear-gradient(135deg,#1C0A00,#3D1200)", padding: "1.3rem 2rem", color: "#FED7AA" }}>
                  ...
                  <div style={{ fontSize: 12, opacity: 0.75 }}>
                    {tableData.orders.length} batch{tableData.orders.length !== 1 ? "es" : ""} · {tableItemCount} item{tableItemCount !== 1 ? "s" : ""}
                  </div>
                </div>
                ...
                <div style={{ padding: "1.25rem 2rem", maxHeight: 340, overflowY: "auto" }}>
                  {tableData.orders.map((batch, bi) => (
                    <div key={batch.batchId} style={{ marginBottom: "1rem" }}>
                      <div style={{ fontSize: 11, color: "#9A6046", fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 5 }}>
                        Batch #{bi + 1} · {batch.time}
                      </div>
                      {batch.items.map((bItem) => (
                        <div key={bItem.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 0", borderBottom: "1px dashed #FDE8D8" }}>
                          ...
                          <div style={{ fontFamily: "Syne", fontWeight: 700, color: "#EA580C", fontSize: 14 }}>
                            ${(bItem.price * bItem.qty).toFixed(2)}
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div style={{ margin: "0 2rem", background: "#FFFBF5", borderRadius: 12, padding: "1rem" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#9A6046", marginBottom: 7 }}>
                    <span>Subtotal</span>
                    <span style={{ fontWeight: 600, color: "#1C0A00" }}>${tableSubtotal.toFixed(2)}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#9A6046", marginBottom: 12 }}>
                    <span>Tax (8%)</span>
                    <span style={{ fontWeight: 600, color: "#1C0A00" }}>${tableTax.toFixed(2)}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", fontFamily: "Syne", fontWeight: 800, fontSize: "1.2rem", paddingTop: 10, borderTop: "2px dashed #F97316" }}>
                    ...
                    <span style={{ color: "#EA580C" }}>${tableGrand.toFixed(2)}</span>
                  </div>
                </div>
                ...
                <div style={{ padding: "1.1rem 2rem 1.6rem", display: "flex", gap: 10 }}>
                  <button
                    ...
                    style={{ flex: 1, background: "#fff", color: "#9A6046", border: "1.5px solid #F3D5BE", padding: "12px", borderRadius: 12, fontFamily: "Syne", fontWeight: 700, fontSize: 14, cursor: "pointer" }}
                  >
                    ← Back
                  </button>
                  <button
                    onClick={confirmBillout}
                    style={{ flex: 2, background: "linear-gradient(135deg,#1C0A00,#3D1200)", color: "#FED7AA", border: "none", padding: "12px", borderRadius: 12, fontFamily: "Syne", fontWeight: 700, fontSize: 14, cursor: "pointer", letterSpacing: "0.4px" }}
                  >
